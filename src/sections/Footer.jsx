    ...
    <footer className="max-container">
      <div className="flex flex-wrap max-lg:flex-col justify-between items-start gap-20 text-coral-red">
        <div className="flex flex-col items-start">
          ...
          <p className="mt-6 text-base font-montserrat leading-6 text-white-400 sm:max-w-sm">
            Order now while still in stock. <br /> Every order above $150 will receive free 5ml sample of Maison Francis Kurkdjian. 
          </p>

          <div className="flex items-center gap-5 mt-8">
            {socialMedia.map((icon) =>(
              <div className="flex justify-center items-center w-10 h-10 bg-white rounded-md">
            ...
            <div key={section}>
              ...
              <ul className="mt-4">
                {section.links.map((link) =>(
                  <li 
                    className="mt-2 text-slate-gray cursor-pointer"
                    key={link.name}
                    >
                    <a>{link.name}</a>
                  </li>
